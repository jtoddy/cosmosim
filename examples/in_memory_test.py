...
def random_object(density=4000, min_mass=0.1*ME, max_mass=1000*ME, 
                  dmin=0, dmax=50*AU,
                  phimin=0, phimax=2*math.pi,
                  thetamin=-math.pi/8, thetamax=math.pi/8,
                  min_velocity=0, max_velocity=5e4, 
                  name=None, color=None):
    mass = random.randint(min_mass, max_mass)
    pspherical = [
        random.randint(dmin,dmax), 
        thetamin + (random.random()*(thetamax)), 
        phimin + (random.random()*(phimax))
    ]
